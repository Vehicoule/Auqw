...
async function socketPair(): Promise<{
  ...
  // Attach the connect waiter BEFORE any await: on a loopback dial the
  // client's 'connect' can fire before the server's 'connection', and a
  // listener attached after that would never run — hanging the pair.
