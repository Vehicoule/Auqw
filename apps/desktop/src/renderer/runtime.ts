...
/**
 * Wall-clock/system shell ports, mirroring the mobile runtime adapter.
 * DOM-free (Date, setTimeout, console, optional crypto) so the module
 * also runs under plain Node for tests.
 */
...
/**
 * `crypto.randomUUID` where the runtime provides it; otherwise a
 * monotonic counter + time + random composite — uniqueness matters,
 * the shape does not.
 */
