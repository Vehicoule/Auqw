...
- **Playing-state surfaces**: search/catalog rows DO mark playing — `toSearchRowModel(meta, i, playingRef)` reads `playback.ref` (published on preparing/buffering/playing/paused) but the memo gates to engaged states only — `playingRef` is null on idle/paused/failed, so a paused catalog row drops the mark + eq overlay. Queue/collection rows use `playing: current && queue.mode === 'playing'` — same semantics on both surfaces.
