...
- **StageSheet**: player/lyrics/queue tabs and transport buttons ARE pressable; the transport 'download' button works. Pan-down collapse is NOT exercisable — synthetic drags only select text (RNGH pan doesn't track mouse on web). Expand it last; F5 resets. At 540px width the full layout needs a ~1000px-tall window (art→air→meta→waveform→transport→seg); a ~700px window clips controls below the fold and the sheet does not scroll — `wmctrl -r Auqw -e 0,60,20,540,1020`.
...
- **Fake TagReader contract** (verified end-to-end): export `hasTagReader()` → `true` or settings hides the local rows; `tagPickFolder()` resolves `{treeUri, label}`; `tagEnumerate(treeUri)` entries need `{docId, name, size, mime, modifiedMs}`; `tagFingerprint` → `{docId, fingerprint}|null` per docId; `tagRead` → `{docId, title, artist, album, durationMs, genre}|null`; `docUri` → `file:///harness/<docId>` (the recording's uri — null/empty means `localResults` filters the row out). Flow: settings → 'add local folder' → `local: scan … entries=N added=N` in the sink → search the title → row renders with `local` provenance note LEADING the sub-line (`local · artist · len`). A malformed entry makes the scan swallow silently (no log, no UI feedback) — inspect the fake, not the click.
...
- **Playing-state surfaces**: search/catalog rows DO mark playing — `toSearchRowModel(meta, i, playingRef)` reads `playback.ref`, which SessionPlayback publishes on preparing/buffering/playing/paused (null on idle/failed). Queue/collection rows use `playing: current && queue.mode === 'playing'`. Nuance: paused keeps the results-row mark + eq overlay (ref still published) while queue rows drop it — same paused moment, different surfaces; confirm intent before calling it a defect.
...
- **Offline → unavailable rows**: toggling the harness 'online' button off marks unowned recordings 'unavailable' — visible on recording-backed rows (StageSheet queue seg NOW PLAYING shows the state note ALONE replacing `artist · len` + warning glyph; search/catalog rows unaffected). Toggle back → reverts live. The 'metered' button does not mark rows.
- Expected harness noise: `queue projection failed` warns (expo-audio provisional player → diagnostics `persistence: failed`), `downloads: … file vanished — degrading to streaming` on reload (in-memory fs + integrity sweep). The `persistence: failed` diagnostic can coexist with working recording/folder-grant persistence across reload (different stores) — don't read it as 'nothing persists'.
